// ⚠️ REMPLACE CES 3 VALEURS PAR LES TIENNES (trouvables dans la console Appwrite)
const PROJECT_ID = '6aa30ee1002fdf9e18de'; 
const DATABASE_ID = 'jobdiagnose_db'; 
const API_KEY = 'standard_'; // Clé API avec TOUTES les permissions cochées

const BASE_URL = `https://cloud.appwrite.io/v1/databases/${DATABASE_ID}`;
const HEADERS = {
    'X-Appwrite-Project': PROJECT_ID,
    'X-Appwrite-Key': API_KEY,
    'Content-Type': 'application/json'
};

const delay = (ms) => new Promise(resolve => setTimeout(resolve, ms));

async function createCollection() {
    console.log('1. Création de la collection "pricing"...');
    const res = await fetch(`${BASE_URL}/collections`, {
        method: 'POST',
        headers: HEADERS,
        body: JSON.stringify({
            collectionId: 'pricing',
            name: 'Tarifs',
            permissions: ['read("any")', 'create("any")', 'update("any")', 'delete("any")'],
            documentSecurity: false
        })
    });
    const data = await res.json();
    if (data.$id || data.message?.includes('already exists')) {
        console.log('✅ Collection "pricing" prête !');
        return true;
    } else {
        console.error('❌ Erreur création collection:', data.message);
        return false;
    }
}

async function createAttributes() {
    console.log('2. Création des attributs...');
    const attributes = [
        { type: 'string', body: { key: 'plan', size: 50, required: true } },
        { type: 'string', body: { key: 'name', size: 100, required: true } },
        { type: 'float', body: { key: 'price', required: true } },
        { type: 'integer', body: { key: 'analyses', required: true } },
        { type: 'string', body: { key: 'description', size: 500, required: true } },
        { type: 'string', body: { key: 'features', size: 255, array: true, required: true } },
        { type: 'boolean', body: { key: 'popular', required: true, default: false } },
        { type: 'integer', body: { key: 'order', required: true } }
    ];

    for (const attr of attributes) {
        const res = await fetch(`${BASE_URL}/collections/pricing/attributes/${attr.type}`, {
            method: 'POST',
            headers: HEADERS,
            body: JSON.stringify(attr.body)
        });
        const data = await res.json();
        if (data.key || data.message?.includes('already exists')) {
            console.log(`  ✅ Attribut "${attr.body.key}" créé.`);
        } else {
            console.error(`  ❌ Erreur attribut "${attr.body.key}":`, data.message);
        }
        await delay(600); // Petit délai pour éviter les limites de vitesse d'Appwrite
    }
    console.log('✅ Tous les attributs sont prêts !');
}

async function createDocuments() {
    console.log('3. Création des documents (tarifs)...');
    const plans = [
        {
            documentId: 'decouverte', plan: 'decouverte', name: 'Découverte', price: 0, analyses: 3,
            description: 'Pour tester le service',
            features: ['3 analyses de CV', 'Rapport PDF basique', 'Score et conseils IA'],
            popular: false, order: 1
        },
        {
            documentId: 'essentiel', plan: 'essentiel', name: 'Essentiel', price: 5.99, analyses: 10,
            description: 'Pour les chercheurs d\'emploi actifs',
            features: ['10 analyses de CV', 'Rapport PDF professionnel 3 pages', 'Matching offre d\'emploi', 'Support email'],
            popular: true, order: 2
        },
        {
            documentId: 'premium', plan: 'premium', name: 'Premium', price: 49.99, analyses: 30,
            description: 'Pour les professionnels exigeants',
            features: ['30 analyses de CV', 'Rapport PDF avancé', 'Matching offre illimité', 'Rédaction CV par expert humain', 'Support prioritaire 24/7'],
            popular: false, order: 3
        }
    ];

    for (const p of plans) {
        const res = await fetch(`${BASE_URL}/collections/pricing/documents`, {
            method: 'POST',
            headers: HEADERS,
            body: JSON.stringify({
                documentId: p.documentId,
                data: p,
                permissions: ['read("any")', 'update("any")', 'delete("any")']
            })
        });
        const data = await res.json();
        if (data.$id || data.message?.includes('already exists')) {
            console.log(`  ✅ Tarif "${p.name}" créé.`);
        } else {
            console.error(`  ❌ Erreur tarif "${p.name}":`, data.message);
        }
        await delay(600);
    }
    console.log('✅ Tous les tarifs sont en ligne !');
}

async function main() {
    if (PROJECT_ID === 'TON_PROJECT_ID') {
        console.log('⚠️ ERREUR : Tu dois remplacer TON_PROJECT_ID, TON_DATABASE_ID et TA_CLE_API_SECRETE dans le fichier avant de lancer le script !');
        return;
    }
    const success = await createCollection();
    if (success) {
        await delay(1000);
        await createAttributes();
        await delay(1500);
        await createDocuments();
    }
    console.log('\n🎉 Configuration Appwrite terminée avec succès !');
}

main();